...
-- --------------------------------------
-- NPC / ROC: same 1v1 PvP loop as player PvP (favorite vs scripted creature, midpoint spawn)
-- --------------------------------------

local function startNpcStylePvpBattle(requester, anchorPosition, redCreatureId, redLevel, redVariant, remoteOpponentName)
	remoteOpponentName = remoteOpponentName or "Roc's creature"
	if activeBattle then
		if pvpEvents.PvPBattleReject then
			pvpEvents.PvPBattleReject:FireClient(requester, "A PvP battle is already in progress.")
		end
	...
	local reqRoot = getPlayerRoot(requester)
	if not reqRoot or not anchorPosition then
		if pvpEvents.PvPBattleReject then
			pvpEvents.PvPBattleReject:FireClient(requester, "Could not place battle.")
		end
		return
	end
	local favReq = PlayerDataManager.GetFavorite(requester)
	...
	print("[PvP] Starting NPC-style PvP: " .. requester.Name .. " vs " .. tostring(redCreatureId))
	activeBattle = true

	if FavoriteCreatureSystem then
		pcall(function()
			FavoriteCreatureSystem.DespawnCompanion(requester)
		end)
	end

	local mid = (reqRoot.Position + anchorPosition) * 0.5
	local dir = (anchorPosition - reqRoot.Position) * Vector3.new(1, 0, 1)
	...
	local battleFolder = Instance.new("Folder")
	battleFolder.Name = "PvPBattle_Npc_" .. requester.UserId
	battleFolder.Parent = workspace
	...
	local infoReq = CreatureData.GetById(favReq.id)
	local infoTarg = CreatureData.GetById(redCreatureId)
	if not infoReq or not infoTarg then
		activeBattle = false
		battleFolder:Destroy()
		if FavoriteCreatureSystem then
			pcall(function()
				FavoriteCreatureSystem.SpawnCompanion(requester)
			end)
		end
		if pvpEvents.PvPBattleReject then
			pvpEvents.PvPBattleReject:FireClient(requester, "Invalid creature data.")
	...
	local statsReq = PlayerDataManager.GetEffectiveStats
			and PlayerDataManager.GetEffectiveStats(favReq.id, favReq.level or 1, favReq.variant or "Normal", requester)
	if not statsReq then
		statsReq = { health = infoReq.health, attack = infoReq.attack, defense = infoReq.defense, speed = infoReq.speed }
		local lvl = favReq.level or 1
		local mult = 1 + (GameConfig.StatGainPerLevel or 0.08) * (lvl - 1)
	...
	local statsTarg = PlayerDataManager.GetEffectiveStats(redCreatureId, redLevel or 1, redVariant or "Normal", nil)
	if not statsTarg then
		statsTarg = { health = infoTarg.health, attack = infoTarg.attack, defense = infoTarg.defense, speed = infoTarg.speed }
		local lvl = redLevel or 1
		local mult = 1 + (GameConfig.StatGainPerLevel or 0.08) * (lvl - 1)
		statsTarg.health = math.floor(statsTarg.health * mult)
		statsTarg.attack = math.floor(statsTarg.attack * mult)
		statsTarg.defense = math.floor(statsTarg.defense * mult)
	...
	blueEntry.model = spawnPvPCreature(favReq.id, bluePoint, "blue", battleFolder, 1, redPoint.Position)
	redEntry.model = spawnPvPCreature(redCreatureId, redPoint, "red", battleFolder, 1, bluePoint.Position)
	if not blueEntry.model or not redEntry.model then
		activeBattle = false
		battleFolder:Destroy()
		if FavoriteCreatureSystem then
	...
	if pvpEvents.PvPBattleStart then
		pvpEvents.PvPBattleStart:FireClient(requester, requester.Name, remoteOpponentName, 0)
	end

	task.wait(2)

	local winnerPlayer, loserPlayer = run1v1Battle(battleFolder, { blueEntry }, { redEntry }, requester, nil)

	if
		(winnerPlayer or loserPlayer)
		and PvPLeaderboardSystem
		and PvPLeaderboardSystem.RecordPvPResult
		and winnerPlayer
		and loserPlayer
	then
		pcall(function()
	...
	if winnerPlayer and winnerPlayer.Parent then
		PlayerDataManager.AddCoins(winnerPlayer, WIN_GOLD)
		if PlayerDataManager.AddXP then
			PlayerDataManager.AddXP(winnerPlayer, favReq.uid, GameConfig.BattleWinXP or 40)
		end
		local d = PlayerDataManager.GetData(winnerPlayer)
		if d then
			local ev = ReplicatedStorage:FindFirstChild("Events")
			local coinsEvt = ev and ev:FindFirstChild("CoinsUpdate")
			if coinsEvt then
				coinsEvt:FireClient(winnerPlayer, d.coins)
			end
		end
	end
	if loserPlayer and loserPlayer.Parent then
		...
		if d then
			...
			local coinsEvt = ev and ev:FindFirstChild("CoinsUpdate")
			if coinsEvt then
				coinsEvt:FireClient(loserPlayer, d.coins)
			end
		end
	end
	...
	local endWinnerName
	if winnerPlayer and winnerPlayer.Parent then
		endWinnerName = winnerPlayer.Name
	else
		endWinnerName = remoteOpponentName
	end
	if pvpEvents.PvPBattleEnd then
		pvpEvents.PvPBattleEnd:FireClient(
			requester,
			endWinnerName,
			winnerPlayer == requester,
			WIN_GOLD,
			LOSER_GOLD_LOSS
		)
	end
	...
	if FavoriteCreatureSystem then
		...
		pcall(function()
			if loserPlayer and loserPlayer.Parent then
				FavoriteCreatureSystem.SetPvPFainted(loserPlayer, true)
				local loserFav = PlayerDataManager.GetFavorite(loserPlayer)
				local loserInfo = loserFav and CreatureData.GetById(loserFav.id)
				local creatureName = (loserInfo and loserInfo.displayName) or "Creature"
				if pvpEvents.PvPRevivePrompt then
					pvpEvents.PvPRevivePrompt:FireClient(loserPlayer, creatureName, coinCost, gemCost)
	...
	activeBattle = false
	print(
		"[PvP] NPC-style battle over. endWinner=" .. tostring(endWinnerName) .. " (+/-" .. WIN_GOLD .. "/" .. LOSER_GOLD_LOSS .. ")"
	)
end

--- Same rules/UI as 1v1 PvP vs a player: favorite vs a scripted creature. anchorPosition = world pos (e.g. Roc HRP).
function PvPBattleSystem.StartNpcStylePvp(requester, params)
	if not requester or not params then
		return false, "Invalid battle."
	end
	if not PlayerDataManager then
		return false, "Battle system not ready."
	end
	if not params.creatureId or not params.anchorPosition then
		return false, "Invalid npc battle request."
	end
	if activeBattle then
		return false, "A PvP battle is already in progress."
	end
	local reqRoot = getPlayerRoot(requester)
	if not reqRoot then
		return false, "You need a character."
	end
	local hum = requester.Character and requester.Character:FindFirstChildOfClass("Humanoid")
	if not hum or hum.Health <= 0 then
		return false, "You can't battle right now."
	end
	if not PlayerDataManager.GetFavorite(requester) then
		return false, "Set a favorite creature in your inventory to PvP."
	end
	if workspace:GetAttribute("ArenaBattleInProgress") then
		return false, "Wait for the arena battle to finish."
	end
	local maxRange = params.maxRangeFromRoc or 80
	if (reqRoot.Position - params.anchorPosition).Magnitude > maxRange then
		return false, "Move closer to Roc."
	end

	local oName = params.opponentName
	local cId = params.creatureId
	local lvl = tonumber(params.level) or 1
	local var = params.variant or "Normal"
	local anchor = params.anchorPosition
	task.spawn(function()
		startNpcStylePvpBattle(requester, anchor, cId, lvl, var, oName)
	end)
	return true, ""
end
...
-- Roc / quick-match: send the standard PvP challenge invite to the nearest valid opponent.
function PvPBattleSystem.ChallengeNearestOpponent(requester, maxDistance)
	if not requester or not PlayerDataManager then
		return false, "Unavailable.", nil
	end
	maxDistance = maxDistance or INTERACTION_RANGE
	if activeBattle then
		return false, "A PvP battle is already in progress.", nil
	end

	local reqRoot = getPlayerRoot(requester)
	if not reqRoot then
		return false, "You need a character.", nil
	end
	local humReq = requester.Character and requester.Character:FindFirstChildOfClass("Humanoid")
	if not humReq or humReq.Health <= 0 then
		return false, "You can't battle right now.", nil
	end
	local favReq = PlayerDataManager.GetFavorite(requester)
	if not favReq then
		return false, "Set a favorite creature to PvP.", nil
	end

	local arenaInProgress = workspace:GetAttribute("ArenaBattleInProgress")
	if arenaInProgress then
		return false, "Wait for the arena battle to finish.", nil
	end

	local bestP, bestDist = nil, maxDistance
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= requester then
			local root = getPlayerRoot(p)
			local hum = p.Character and p.Character:FindFirstChildOfClass("Humanoid")
			if root and hum and hum.Health > 0 then
				local d = (root.Position - reqRoot.Position).Magnitude
				if d <= bestDist then
					bestDist = d
					bestP = p
				end
	...
	if not bestP then
		return false, "No other players within " .. math.floor(maxDistance) .. " studs.", nil
	end
	local favTarg = PlayerDataManager.GetFavorite(bestP)
	if not favTarg then
		return false, bestP.Name .. " has no favorite creature set.", nil
	end

	local eventsFolder = ReplicatedStorage:FindFirstChild("Events")
	local invite = eventsFolder and eventsFolder:FindFirstChild("PvPChallengeInvite")
	if not invite then
		return false, "PvP invites unavailable.", nil
	end

	pendingChallenges[bestP.UserId] = requester.UserId
	invite:FireClient(bestP, requester.UserId, requester.Name)
	return true, nil, bestP.Name
end
